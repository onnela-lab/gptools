...
def test_lattice_predecessors(
    ...
    # Shape check for up to two dimensions if `k` is a scalar and the predecessors are
    # compressed.
...
def test_lattice_predecessors_invalid(
    shape: Tuple[int], ks: Tuple[int], bounds: graph.LatticeBounds, match: str
) -> None:
...
def test_predecessors_to_edge_index(indexing: Literal["numpy", "stan"]) -> None:
    ...
    # The number of edges is the number of all combinations less the neighbors that fall
    # outside the bounds of the tensor. We just do a weak check here.
...
def test_predecessors_to_edge_index_invalid(
    predecessors: np.ndarray, match: str
) -> None:
...
@pytest.mark.parametrize(
    "edge_index, indexing, match",
    [
        (np.zeros(2), None, "edge index must have shape (2, num_edges) but got (2,)"),
        (
            np.zeros((3, 2)),
            None,
            "edge index must have shape (2, num_edges) but got (3, 2)",
        ),
...
def test_check_edge_index_invalid(
    edge_index: np.ndarray, indexing: Literal["numpy", "stan"], match: Optional[str]
) -> None:
    with pytest.raises(ValueError, match=re.escape(match)):
...
def test_edge_index_to_graph_roundtrip(return_mapping: bool) -> None:
    ...
    edge_index = graph.graph_to_edge_index(
        G, indexing="numpy", return_mapping=return_mapping
    )
    if return_mapping:
