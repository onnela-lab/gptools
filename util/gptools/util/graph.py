...
# Solutions to the Gauss circle problem
# (https://en.wikipedia.org/wiki/Gauss_circle_problem).
GAUSS_PROBLEM_SEQUENCE = np.asarray(
...
class LatticeBounds(enum.Enum):
    ...
    - DIAMOND results in the highest-volume hypercuboid whose vertices are axis-aligned
        that can fit into CUBE.
...
def lattice_predecessors(
    ...
    bounds: LatticeBounds = LatticeBounds.ELLIPSE,
    compress: bool = True,
) -> np.ndarray:
    """
    Evaluate predecessors for nodes on a lattice with given window size. Approximations
    will generally be poor if the half window width is smaller than the correlation
    length of the kernel.

    Args:
        shape: Shape of the tensor with Gaussian process distribution.
        k: Half window width or sequence of half window widths for each dimension. Each
            node will have a receptive field at most :code:`k` to the "left" and "right"
            in each dimension.
        bounds: Bounds of the receptive field. See :class:`LatticeBounds` for details.
        compress: Whether to compress predecessors such that the number of colums is
            equal to the maximum degree.

    Returns:
        predecessors: Mapping from each element of the tensor to its predecessors with
            shape :code:`(prod(shape), l)`, where :code:`l` is the number of
            predecessors.
    """
    ...
    for i, (size, width) in enumerate(zip(shape, 2 * k + 1)):
        if width > size:
            raise ValueError(
                f"window width {width} exceeds the tensor size {size} along dim {i}"
            )
    ...
    # Construct the vector predecessors and a mask that removes indices outside the
    # tensor bounds.
    ...
    if bounds == LatticeBounds.ELLIPSE:
        ...
        mask &= t <= 1
    # Mask invalid indices, ravel the coordinate indices, and mask again after
    # ravelling.
...
def compress_predecessors(predecessors: np.ndarray) -> np.ndarray:
    """
    Compress a predecessor matrix such that there is at least one predecessors that does
    not contain invalid indices. In other words, we remove as many columns as possible
...
def predecessors_to_edge_index(
    predecessors: np.ndarray, indexing: Literal["numpy", "stan"] = "stan"
) -> np.ndarray:
    """
    Convert a matrix of predecessors to an edgelist with self loops.

    Args:
        predecessors: Predecessor matrix such that each row corresponds to the
            predecessors of the associated node. Negative node labels are omitted.
        indexing: Whether to use zero-based indexing (`numpy`) or one-based indexing
            (`stan`).
...
def check_edge_index(
    edge_index: np.ndarray, indexing: Literal["numpy", "stan"] = "stan"
) -> np.ndarray:
    """
    Check the edge index, ensuring it has the right structure for the Stan
    implementation and corresponds to a directed acyclic graph.
    ...
    if edge_index.ndim != 2 or edge_index.shape[0] != 2:
        raise ValueError(
            f"edge index must have shape (2, num_edges) but got {edge_index.shape}"
        )
    ...
    if (actual_min := edge_index.min()) < expected_min:
        raise ValueError(
            f"expected indexing to start at {expected_min}; found {actual_min}"
        )
