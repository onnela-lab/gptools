...
def _run_main(tmp_path: pathlib.Path, parameterization: str, noise_scale: float = 1.0, **kwargs):
    ...
    __main__([parameterization, str(noise_scale), str(path), *dict2args(**kwargs)])
...
def test_profile_timeout(tmp_path: pathlib.Path) -> None:
    result, kwargs = _run_main(tmp_path, "centered", iter_sampling=100_000, timeout=0.7)
...
def test_profile_max_chains(tmp_path: pathlib.Path) -> None:
    result, kwargs = _run_main(tmp_path, "centered", max_chains=7)
