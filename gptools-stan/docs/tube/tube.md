...
```{code-cell} ipython3
model = compile_model(stan_file="tube.stan")
...
rate = fit.stan_variable("rate").mean(axis=0)
...
a = fit.stan_variable("length_scale")
b = fit.stan_variable("sigma")
pts = ax2.scatter(a, b, marker=".", c=fit.method_variables()["lp__"])
d = fit.method_variables()["divergent__"].astype(bool).squeeze()
...
c = fit.stan_variable("eta").mean(axis=0)
vmax = np.abs(c).max()
...
effect = fit.stan_variable("degree_effect")
...
effect = fit.stan_variable("zone_effect")
...
ax2.text(0.025, 0.05, "(b)", transform=ax2.transAxes)
ax3.text(0.05, 0.95, "(c)", transform=ax3.transAxes, va="top")
